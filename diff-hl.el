           ...
           (dolist (buf (buffer-list))
             (with-current-buffer buf
               (when (and diff-hl-mode
                          (not (local-variable-p 'diff-hl-reference-revision)))
...
effect."
  ...
  (let ((name (project-name (project-current))))
    (cond
     ;; reset
     (current-prefix-arg
      (diff-hl-set-reference-rev-in-project-internal nil)
      (message "Reference revision reset to the most recent revision (project %s)" name))
     ;; set
     (t (diff-hl-set-reference-rev-in-project-internal rev)
        (message "Showing changes against %s (project %s)" rev name)))))

(defun diff-hl-set-reference-rev-in-project-internal (rev)
  (let* ((proj (project-current))
         (class (intern (concat "diff-hl-" (project-root proj)))))
...
per-project reference created by `diff-hl-set-reference-rev-in-project'."
  ...
  (dolist (buf (buffer-list))
    (with-current-buffer buf
      (when diff-hl-mode
        (when arg (kill-local-variable 'diff-hl-reference-revision))
